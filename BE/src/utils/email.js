...
const sendMailWithTimeout = (transporter, mailOptions, timeoutMs = 50000) =>
  Promise.race([
    transporter.sendMail(mailOptions),
    new Promise((_, reject) =>
      setTimeout(() => reject(new Error(`SMTP timeout sau ${timeoutMs / 1000}s`)), timeoutMs)
    ),
  ]);
...
/**
 * Gửi email chứa mã OTP (dùng cho cả đặt lại mật khẩu lẫn xác thực đăng ký)
 * @param {string} toEmail - Email người nhận
 * @param {string} otp - Mã OTP 6 chữ số
 * @param {string} [type='register'] - Loại OTP: 'register' | 'reset'
...
const sendOTPEmail = async (toEmail, otp, type = 'register') => {
  ...
  const subject = isReset
    ? '🔐 Mã OTP đặt lại mật khẩu Plantify của bạn'
    : '🌱 Mã OTP đăng ký tài khoản Plantify của bạn';
  ...
  const bodyText = isReset
    ? 'Chúng tôi nhận được yêu cầu đặt lại mật khẩu cho tài khoản Plantify của bạn. Vui lòng sử dụng mã OTP dưới đây để hoàn tất việc xác thực.'
    : 'Chào mừng bạn đến với Plantify! Bạn vừa tạo một tài khoản mới với địa chỉ email này. Vui lòng sử dụng mã OTP dưới đây để hoàn tất việc xác thực và kích hoạt tài khoản.';
  ...
  console.log('\n==================================================');
  console.log(`[EMAIL SENDING] Gửi mã OTP ${isReset ? 'đặt lại mật khẩu' : 'đăng ký tài khoản'} cho: ${toEmail}`);
  console.log(`[OTP CODE] Mã OTP: ${otp}`);
  console.log('==================================================\n');

  try {
    await sendMailWithTimeout(transporter, mailOptions);
    console.log(`✅ Gửi email OTP tới ${toEmail} thành công.`);
  } catch (error) {
