  ...
  return <Dialog open={open} onOpenChange={(nextOpen) => { if (!userPlantSaving) onOpenChange(nextOpen); }}>
    <DialogContent className="max-h-[90vh] overflow-y-auto sm:max-w-2xl">
      ...
      <form data-testid="user-plant-form" className="space-y-4 rounded-xl border p-4" onSubmit={handleUserPlantSubmit}>
        ...
        <DialogFooter><Button type="button" variant="outline" disabled={userPlantSaving} onClick={() => onOpenChange(false)}>Hủy</Button><Button type="submit" disabled={userPlantSaving}>{userPlantSaving ? <Loader2 className="mr-2 h-4 w-4 animate-spin" /> : null}{editing ? "Lưu" : "Tạo cây"}</Button></DialogFooter>
      </form>
      ...
      {editing && workingPlant ? <div data-testid="user-plant-independent-sections" className="space-y-4"><UserPlantAlbum userPlant={workingPlant} onChanged={handleUserPlantChanged} /><UserPlantScheduleSettings userPlant={workingPlant} onChanged={handleUserPlantChanged} /><UserPlantCareEvents userPlantId={workingPlant._id} onRecorded={handleCareRecorded} /></div> : null}
    </DialogContent>
  </Dialog>;
}
