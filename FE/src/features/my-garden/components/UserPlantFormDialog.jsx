...
export function UserPlantFormDialog({ open, onOpenChange, userPlant, catalogPlants, catalogLoading, catalogError, saving, onSubmit, onUserPlantChanged }) {
  ...
  return <Dialog open={open} onOpenChange={(nextOpen) => { if (!userPlantSaving) onOpenChange(nextOpen); }}>
    <DialogContent className="max-h-[90vh] overflow-y-auto sm:max-w-2xl">
      ...
      {editing && workingPlant ? <div data-testid="user-plant-independent-sections" className="space-y-4"><UserPlantAlbum userPlant={workingPlant} onChanged={handleAlbumChanged} /><UserPlantCareEvents userPlantId={workingPlant._id} /></div> : null}
    </DialogContent>
  </Dialog>;
}
